...
def _series_by_agent(
    df_ep: pd.DataFrame, col: str
...
def plot_aggregate(csv_path: Path, out_dir: Path):
    ...
    shared = (
        df.sort_values(["episode", "col_idx", "agent"])
        .groupby(["episode", "col_idx"])
        .first()
        .reset_index()
    )
    pg_stats = (
        shared.groupby("col_idx")["pg_total"]
        .agg(["mean", "std"])
        .reset_index()
        .sort_values("col_idx")
    )
    price_stats = (
        shared.groupby("col_idx")["price"]
        .agg(["mean", "std"])
        .reset_index()
        .sort_values("col_idx")
    )
    ...
    ax1 = plt.gca()
    ax1.plot(
        pg_stats["col_idx"].to_numpy(), pg_stats["mean"].to_numpy(), color="tab:blue"
    )
    ax1.set_xlabel("Hour index")
    ...
    ax2 = ax1.twinx()
    ax2.plot(
        price_stats["col_idx"].to_numpy(),
        price_stats["mean"].to_numpy(),
        color="tab:orange",
    )
