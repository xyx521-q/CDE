...
def run_sequential(args, logger):
    ...
    has_continuous_actions = (
        "action_spaces" in env_info
        and all(isinstance(act_space, BOX_SPACES + TUPLE_SPACES) for act_space in env_info["action_spaces"])
    )
    if not has_continuous_actions:
        ...
        if all([isinstance(act_space, BOX_SPACES) for act_space in args.action_spaces]):
            for _aid in range(args.n_agents):
                for _actid in range(args.action_spaces[_aid].shape[0]):
                    ...
                    mult_coef_tensor[_aid, _actid] = np.asarray(_action_max - _action_min).item()
                    action_min_tensor[_aid, _actid] = np.asarray(_action_min).item()
        elif all([isinstance(act_space, TUPLE_SPACES) for act_space in args.action_spaces]):
            for _aid in range(args.n_agents):
                ...
                for _actid in range(args.action_spaces[_aid].spaces[1].shape[0]):
                    ...
                    mult_coef_tensor[_aid, tmp_idx] = np.asarray(_action_max - _action_min).item()
                    action_min_tensor[_aid, tmp_idx] = np.asarray(_action_min).item()
    ...
    while runner.t_env <= args.t_max:
        ...
        if (runner.t_env - last_test_T) / args.test_interval >= 1.0:
            ...
            logger.log_stat("eval/rl_reward", rl_eval_reward, runner.t_env, to_sacred=False)
            logger.log_stat("eval/rl_win_rate", rl_battle_win_rate, runner.t_env, to_sacred=False)
            if args.EA:
                ea_eval_reward = 0
            ...
            logger.log_stat("eval/ea_reward", ea_eval_reward, runner.t_env, to_sacred=False)
            logger.log_stat("eval/ea_win_rate", ea_battle_win_rate, runner.t_env, to_sacred=False)
            logger.log_stat("eval/best_reward", max(rl_eval_reward, ea_eval_reward), runner.t_env, to_sacred=False)
            logger.log_stat("eval/best_win_rate", max(rl_battle_win_rate, ea_battle_win_rate), runner.t_env, to_sacred=False)
            # our_wandb.log(
        ...
        if (runner.t_env - last_log_T) >= args.log_interval:
            ...
            logger.log_stat("train/episode", episode, runner.t_env, to_sacred=False)
            logger.log_stat("train/t_env", runner.t_env, runner.t_env, to_sacred=False)
            if hasattr(buffer, "episodes_in_buffer"):
                logger.log_stat("buffer/episodes", buffer.episodes_in_buffer, runner.t_env, to_sacred=False)
