...
class EpisodeBatch:
    ...
    def update(self, data, bs=slice(None), ts=slice(None), mark_filled=True):
        ...
        for k, v in data.items():
            ...
            if th.is_tensor(v):
                ...
                # NumPy first combines lists of arrays without PyTorch's slow path.
    ...
    def _parse_slices(self, items):
        ...
        if (
            isinstance(items, (slice, int, list, np.ndarray))
            or th.is_tensor(items)
        ):
        ...
        for col in cols:
            if self.data.transition_data[col].dim() == 4:
                ...
                for aid in range(n_agents):
                    cln_cols.append(col + f"__agent{aid}")
                    cln_data.append(
                        self.data.transition_data[col][:, :, aid, :].cpu().numpy()
                    )
            else:
                cln_cols.append(col)
        ...
        if self.data.episode_data != {}:
            for b in range(self.data.episode_data[cols[0]].shape[0]):
                pd = pd.DataFrame(
                    columns=cln_cols,
                    data=[
                        [cln_data[j][b, :] for j, _ in enumerate(cln_cols)]
                        for t in range(1)
                    ],
                )
...
class ReplayBuffer(EpisodeBatch):
    ...
    def __repr__(self):
        return f"ReplayBuffer. {self.episodes_in_buffer}/{self.buffer_size} episodes.q Keys:{self.scheme.keys()} Groups:{self.groups.keys()}"
