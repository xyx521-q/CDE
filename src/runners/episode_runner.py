...
class EpisodeRunner:
    ...
    def reset(self, test_mode=False):
        ...
        if test_mode and hasattr(self.env, "reset_test"):
            self.env.reset_test()
        else:
            self.env.reset()
        self.t = 0
    ...
    def _log(self, returns, stats, prefix):
        ...
        tb_prefix = "test" if prefix == "test_" else "train"

        self.logger.log_stat(prefix + "return_mean", return_mean, self.t_env)
        self.logger.log_stat(prefix + "return_std", return_std, self.t_env)
        self.logger.log_stat("{}/return_mean".format(tb_prefix), return_mean, self.t_env, to_sacred=False)
        self.logger.log_stat("{}/return_std".format(tb_prefix), return_std, self.t_env, to_sacred=False)
        returns.clear()

        for k, v in stats.items():
            if k != "n_episodes":
                stat_value = v / stats["n_episodes"]
                self.logger.log_stat(prefix + k + "_mean" , stat_value, self.t_env)
                self.logger.log_stat("{}/{}".format(tb_prefix, k), stat_value, self.t_env, to_sacred=False)
        stats.clear()
